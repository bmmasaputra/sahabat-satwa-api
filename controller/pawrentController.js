...
async function getPawrentById(req, res) {
  const pawrentId = req.body.pawrent_id;
...
async function updatePawrentData(req, res) {
  const { pawrent_id, nama_lengkap_pawrent, no_telepon_pawrent } = req.body;

  if (!pawrent_id || !nama_lengkap_pawrent || !no_telepon_pawrent) {
    return res.status(400).json({ success: false, message: "All fields required" });
  }
